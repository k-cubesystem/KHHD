...
async function testMigrations() {
  ...
  try {
    const { data, error } = await supabase.from('v_destiny_targets').select('*').limit(1)
  ...
  try {
    const { data, error } = await supabase.from('analysis_history').select('*').limit(1)
  ...
  try {
    ...
    const { error } = await supabase.rpc('get_user_destiny_targets', {
