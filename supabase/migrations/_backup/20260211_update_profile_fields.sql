-- Update profiles table for new address and avatar system
-- Remove image URL fields (use temporary upload instead)
-- Add work_address for separate workplace fengshui analysis
...
SELECT
  ...
  p.gender,
  p.avatar_url,
  NULL::text AS face_image_url,    -- Kept for compatibility, always NULL for profiles
  NULL::text AS hand_image_url,    -- Kept for compatibility, always NULL for profiles
  p.home_address,
  'self' AS target_type,
