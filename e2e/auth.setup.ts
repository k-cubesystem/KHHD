...
setup('authenticate as user', async ({ page }) => {
  ...
  await page.getByLabel('비밀번호', { exact: true }).fill(process.env.E2E_USER_PASSWORD || 'test1234!')
...
setup('authenticate as admin', async ({ page }) => {
  ...
  await page.getByLabel('비밀번호', { exact: true }).fill(process.env.E2E_ADMIN_PASSWORD || 'admin1234!')
  await page.getByRole('button', { name: '로그인', exact: true }).click()
