...
test.describe('우리 가족 기운 지도', () => {
  ...
  test('가족 관리 → 지도 진입 + 오행·평균 렌더', async ({ page }) => {
    ...
    // 본인 1명뿐이면 빈 상태, 2명 이상이면 지도 v2(오각형 + 팩트 세 줄 + 관계 + AI 문)
    const emptyState = page.getByText('아직 견줄 기운이 없습니다')
    const mapHeading = page.getByRole('heading', { name: /우리 가족/ })
    ...
    if (await emptyState.isVisible().catch(() => false)) {
      ...
      await expect(page.getByText(/가장 부족한 기운/)).toBeVisible()
      ...
      expect(body).not.toMatch(/\d+\s*%|\d+\s*점/)

      // 살림 업셀 링크가 상점 아이템 탭으로
      const upsell = page.getByRole('link', { name: /기운 살림 보러가기/ })
      await expect(upsell).toBeVisible()
      expect(await upsell.getAttribute('href')).toContain('/protected/store?tab=items')
      console.log('[PASS] 용신 살림 업셀 → 상점 아이템 탭')
    }
