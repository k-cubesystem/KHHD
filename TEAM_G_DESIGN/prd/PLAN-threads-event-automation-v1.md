...
**(B) 데일리 콘텐츠 글** — 🔴 2026-08-19 CEO 발주로 **하루 2편 이야기 콘텐츠**로 확대·대체됨:
별도 문서 `PLAN-threads-content-engine-v1.md`(「해화지기의 복채 장부」)가 이 항을 대신한다. 아래는 초기안 기록.

**(B-구안) 데일리 콘텐츠 글 (화~금 1건)** — 계정이 «이벤트 봇»이 아니라 «읽을 만한 계정»이 되게 하는 축. 4포맷 로테이션:
