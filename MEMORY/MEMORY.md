...
| 2026-07-25 | 세션31      | 로그인 전 랜딩 전면 개선 — **P0 후기 흰글씨(bg-white+zen.text=#FFFFFF, 대비 1:1) 수복 → 다크 서피스 12.07:1** / 유령토큰 bg-zen-bg 제거·페이드 복구 / 후기 2줄40장→1줄10장·카드 320→268px / 회원수 맨숫자→라벨 문장 / **랜딩 로그인 진입점 0개→3개**(상단 바·히어로·CTA 하단) / 히어로 CTA /protected→/auth/sign-up(무맥락 리다이렉트 제거)·카피 4~5줄→2줄·랜덤시작 제거 / 히어로 오버레이 30·60·90→55·75·90(구 오버레이가 슬라이드2·3에서 WCAG AA 위반 3.06:1 실측) / **몰입형 상세페이지 /story 신설**(7단 서사·리포트 블러 미리보기·sticky CTA, 가격은 formatFeatureCost 단일소스) / 공개 랜딩에 사업자정보 푸터 추가(전자상거래법) / **eslint.config.mjs FlatCompat→네이티브 flat config: 리포 전역 lint 크래시 수복**(그동안 lint 무효였음, 선행 위반 409건 표면화) | app/page.tsx, app/story/, components/landing/{social-proof,landing-auth-bar,review-marquee,live-member-counter,story/}.tsx, components/HeroCarousel.tsx, components/site-footer.tsx, eslint.config.mjs, TEAM_G_DESIGN/prd/PLAN-landing-prelogin-v1.md                                                                                   |
