...
### §7 실행 — 아이콘→일러스트 전량 전환 (사용자 확정: 신당지기=主神 연동·부감15°·E세트 전량·기능아이콘 포함)
...
## 🔮 백로그 진행 — 탭 반응·잠금 티저·신탁 선톡 (2026-07-13, 세션14)

- **§3.2 제단 主神 탭 반응**: 신당지기(=좌정 主神) 탭 시 신위 고유 사운드(aura.sound) + 시그니처 파티클 버스트(EffectsCanvas.burstAura) + 바운스 + 인사말. mainDeity에 sound 필드 추가.
- **잠금 신위 티저**: 신위전 잠금 신위를 그레이스케일 대신 **색 유지(설빛온기 매력 노출) + 🔒 배지 + 한 줄 티저**(「관장영역」의 등급명). 봉안 유도 강화.
- **신탁 선톡(§5.4)**: `deity_oracles` 테이블(RLS: 본인 읽기/확인, 발행 service_role) + `getRoomOracle`/`markOracleSeen` 액션. 방 진입 시 미확인 신탁 우선 노출, 없으면 빈도 상한(2일 1회·주 3회) 판정 후 좌정 主神 페르소나로 짧은 안부 신탁 생성. 방 말풍선에 ✦신탁✦ 강조 표시(골드 글로우). 생성 실패는 조용히 무시(방 비차단).

검증: tsc 0 / jest 161. 배포+프로덕션 재검.

---

## 👤 프로필 재기획 + 도깨비 아바타 폐지 (2026-07-13, 세션15 · 사용자 지시)

### 도깨비 아바타 제거 (프로필 범위)

- `components/profile/avatar-selector.tsx` 삭제, settings-form에서 선택기·핸들러 제거(소셜 이미지만 사용).
- 프로필 페이지 도깨비 렌더 분기 제거. 구 `/avatars/dokkaebi-*` URL은 렌더 시 무시(가드).
- 가족 멤버 도깨비 아바타(`components/family/*`, `lib/constants/dokkaebi-avatars.tsx`)는 별개 컨텍스트로 유지(요청 범위="프로필").

### 프로필 재구성 (DESIGN.md 「반가의 서재」 컨셉)

- **아바타 = 정체성**: 좌정 主神 초상 > 오행 엠블럼(일간 오행, §7 에셋 재사용) > 소셜 이미지 > 모노그램. 도깨비 대신 사주·신당과 통합.
- **정체성 히어로**: 이름 + **사주 정체성 한 줄**(예 "己土 일간 · 흙의 기운", 서버에서 getSajuData 계산) + 멤버십 칩 + 主神 좌정 표기. 생년월일 없으면 입력 유도 CTA.
- **3지표 스트립**: 복채·멤버십·분석기록 수(기존 2카드+중복 정리).
- 출석 → 복허브 → **바로가기 그리드**(나의 신당[강조·신규]·내 명식·인연 관리·사주 기록·멤버십·설정, 기존 중복 nav+설정 통합) → 설정 리스트(알림·개인정보·고객센터) → 로그아웃.
- 전면 한글화(Log Out→로그아웃 등), '나의 서재' 오버라인.

검증: tsc 0 / jest 161. 배포+프로덕션 스크린샷 검증.
