 ...
 *
 * ## 🔴 왜 화면을 따로 만들었나
 * 예전에는 명식이 없어도 **풀이가 그냥 시작됐다.** 로딩 화면이 돌고, 값을 먼저 빼려다
 * 실패하고, 사람에게는 결제 안내가 떴다 — 낼 것이 충분히 있는데도.
 * 문제는 결제가 아니라 **생년월일이 없다는 것**이었는데 화면이 엉뚱한 곳을 가리켰다.
 *
 * 그래서 두 가지를 지킨다 —
 *   ① **이용권 사용보다 먼저** 확인한다. 돌 수 없는 풀이에는 값을 매기지 않는다.
...
export function MissingBirthChart({
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] w-full max-w-md flex-col items-center justify-center px-5 text-center">
      ...
      <p className="mt-3 font-sans text-[11.5px] text-ink-light/35">이용권은 아직 쓰이지 않았습니다.</p>
