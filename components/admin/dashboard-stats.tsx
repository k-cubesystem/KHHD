          ...
          >
            <Card className="relative p-3.5 bg-gradient-to-br from-surface/30 to-surface/20 border border-white/30 hover:border-gold-500/30 transition-all overflow-hidden group">
              ...
              <div className="relative">
                ...
                <p className="text-[9px] text-ink-primary/40 uppercase tracking-wider font-medium mb-1">{card.label}</p>
                <div className="text-xl font-bold text-ink-primary font-mono tabular-nums leading-tight">
                  {card.value}
                </div>
                <p className="text-[10px] text-ink-primary/30 mt-1">{card.sub}</p>
              </div>
            </Card>
          </motion.div>
        )
...
export function AnimatedHeader({ title, subtitle }: AnimatedHeaderProps) {
  return (
    <motion.div
      ...
      className="mb-4"
    >
      <h1 className="text-xl font-serif font-bold text-ink-primary">{title}</h1>
      <p className="text-xs text-ink-primary/40 mt-0.5">{subtitle}</p>
    </motion.div>
  )
}
