...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    timeout: 'bg-purple-500/20 text-purple-400 border-purple-500/30',
    error: 'bg-red-500/20 text-red-400 border-red-500/30',
  }
  const cls = map[status] ?? 'bg-stone-500/20 text-stone-400 border-stone-500/30'
  return (
    <span className={cn('text-[9px] font-bold px-1.5 py-0.5 rounded border', cls)}>
      {status.toUpperCase()}
    </span>
  )
}
...
export function GeminiUsageDashboard({
  ...
  function changePeriod(p: 7 | 30 | 90) {
    setPeriod(p)
    startRefresh(async () => {
      const [newDaily, newAction] = await Promise.all([
  ...
  // ── 요약 카드 데이터 ──────────────────────
  const errorRate =
    summary.total_calls > 0 ? ((summary.error_calls / summary.total_calls) * 100).toFixed(1) : '0.0'
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-0.5">
            오늘 기준 · 환율 ₩{usdKrwRate.toLocaleString()}/USD
          </p>
        </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  period === p
                    ...
                    : 'bg-ink-900 text-stone-400 hover:text-stone-200'
                )}
              >
                {PERIOD_LABELS[p]}
              </button>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-ink-900/60 border border-stone-700/40 rounded-xl p-3.5">
          ...
          <div className="text-[10px] text-stone-500 mt-1">
            입력 {summary.total_input_tokens.toLocaleString()} · 출력{' '}
        ...
        <div className="bg-ink-900/60 border border-stone-700/40 rounded-xl p-3.5">
          ...
          <div className="text-lg font-black text-gold-400 leading-tight">
      ...
      <div className="bg-ink-900/60 border border-stone-700/40 rounded-xl p-4">
          ...
          <div className="text-[10px] text-stone-500 mb-3 space-y-0.5">
            ...
            <div>
              ...
              <span
                className={cn(
                  'font-bold',
                  rpmConfig.tokens < 3 ? 'text-red-400' : 'text-emerald-400'
                )}
              >
        ...
        {rpmMsg && (
          <p
            className={cn(
              'text-[10px] mt-1',
              rpmMsg.startsWith('✓') ? 'text-emerald-400' : 'text-red-400'
            )}
          >
            {rpmMsg}
          </p>
          ...
          <table className="w-full text-[10px]">
            ...
            <tbody>
              ...
              {logs.map((log) => (
                <tr
                  key={log.id}
                  className="border-b border-stone-800/50 hover:bg-ink-800/30 transition-colors"
                >
