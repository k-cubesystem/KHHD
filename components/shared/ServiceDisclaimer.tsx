...
export const AI_DISCLOSURE_TEXT: Record<DisclaimerTone, string> = {
  ...
  /**
   * 속풀이 — 신위를 지우지 않고 층을 나눈다.
   * 🔴 문답 화면은 이 줄이 입력창 위에 «상시» 뜨는 자리라 길면 대화를 가린다(CEO 지시 2026-08-24:
   *    「없어졌으면 좋겠다」). 법이 요구하는 것은 «생성형 인공지능이 생성했다는 사실»의 표시뿐이므로
   *    두 문장(43자)을 한 문장(19자)으로 줄였다 — 법정 문언은 그대로 두고 길이만 깎은 것이다.
   *    더 줄이거나 없애는 것은 §31② 위반 소지가 있다(조사 정본 RESEARCH-20260812-ai-basic-act.md).
   */
  chat: '신위의 말은 생성형 인공지능이 냅니다.',
  /** 신탁 오버레이 — 자리가 한 줄뿐이다 */
