    ...
    <div className="relative flex flex-col gap-12 items-center text-center px-4 pt-32 pb-20 z-10">
      ...
      >
        ...
        <h1 className="text-6xl md:text-8xl font-serif font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-b from-white via-white/90 to-white/60 drop-shadow-2xl">
          海華堂<span className="text-2xl md:text-4xl text-gold-500 font-light ml-4 tracking-normal align-middle">AI</span>
        </h1>

        <p className="text-lg md:text-xl text-stone-300 max-w-2xl mx-auto leading-relaxed font-light font-sans">
          데이터 사이언스의 <span className="text-gold-400 font-medium">논리</span>와 전통 명리학의 <span className="text-gold-400 font-medium">통찰</span>.<br />
      ...
      >
        {[
          { title: "天 (운명)", subtitle: "Hyper-Precision Saju", desc: "나노초 단위의 정밀 만세력 엔진", icon: "🌌" },
          { title: "人 (의지)", subtitle: "Multimodal Analysis", desc: "관상과 손금을 읽는 비전 AI", icon: "👤" },
          { title: "地 (환경)", subtitle: "Fengshui Solution", desc: "공간의 에너지를 최적화하는 풍수", icon: "🌏" },
        ].map((item, i) => (
          <motion.div
            key={i}
          ...
          >
            ...
            <p className="text-sm text-stone-400 leading-relaxed z-10">
              {item.desc}
            </p>
          </motion.div>
        ))}
