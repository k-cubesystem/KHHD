    ...
    <div className="relative flex flex-col gap-12 items-center text-center px-4 pt-32 pb-20 z-10">
      ...
      >
        ...
        <h1 className="text-6xl md:text-8xl font-serif font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-b from-white via-white/90 to-white/60 drop-shadow-2xl">
          海華堂
          <span className="text-2xl md:text-4xl text-gold-500 font-light ml-4 tracking-normal align-middle">AI</span>
        </h1>

        <p className="text-lg md:text-xl text-ink-primary max-w-2xl mx-auto leading-relaxed font-light font-sans">
      ...
      >
        {[
          ...
          >
            ...
            <h3 className="text-ink-light font-serif font-bold text-2xl group-hover:text-gold-400 transition-colors">
              {item.title}
            </h3>
            <span className="text-xs font-mono text-gold-500/70 tracking-wider uppercase">{item.subtitle}</span>
            <div className="w-8 h-[1px] bg-white/10 my-2 group-hover:bg-gold-500/50 transition-colors" />
            <p className="text-sm text-ink-light/70 leading-relaxed z-10">{item.desc}</p>
          </div>
        ))}
      </div>
    </div>
