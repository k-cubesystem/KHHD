...
export function Hero2026({ isGuest, masterName: _masterName }: Hero2026Props) {
  return (
      ...
      <div className="relative z-10 flex flex-col items-center justify-center text-center px-6 py-12 md:px-12">
        ...
        >
          ...
          >
            ...
            <span className="text-[10px] font-bold tracking-[0.15em] text-primary uppercase">Haehwadang Premium</span>
          </div>
          ...
          <p className="text-xs md:text-sm text-ink-light/60 font-sans leading-relaxed mb-6 max-w-sm mx-auto">
            ...
            <br />내 인생을 주체적으로 관리할 수 있습니다.
          </p>

          {/* Actions */}
          <div className="flex flex-col sm:flex-row gap-3 justify-center items-center w-full">
            <Link href={isGuest ? '/auth/sign-up' : '/protected/studio'} className="w-full sm:w-auto">
              <button className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-primary text-charcoal-deep font-serif font-bold text-sm rounded-lg shadow-[0_4px_20px_rgba(212,175,55,0.2)] hover:shadow-[0_6px_25px_rgba(212,175,55,0.3)] transition-all active:scale-95">
            ...
            {isGuest && (
              <Link href="/auth/login" className="w-full sm:w-auto">
                <button className="w-full sm:w-auto px-6 py-3 border border-white/10 text-ink-light/70 font-sans text-xs rounded-lg hover:bg-white/5 transition-all">
                  로그인하기
                </button>
              </Link>
            )}
