...
export function DesktopView({ isGuest, masterName, userData: _userData }: DesktopViewProps) {
        ...
        <div className="max-w-[1400px] mx-auto grid grid-cols-12 gap-8">
          ...
          <main className="col-span-12 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Cheonjiin */}
              <Link
                href={isGuest ? '/auth/sign-up' : '/protected/analysis'}
                className="group relative h-64 bg-surface/40 border border-primary/20 rounded-2xl p-8 overflow-hidden hover:border-primary/60 transition-all hover:shadow-[0_0_30px_rgba(236,182,19,0.1)]"
              >
                ...
                <CloudMoon
                  className="absolute -bottom-4 -right-4 w-48 h-48 text-primary/5 group-hover:text-primary/10 transition-colors rotate-12"
                  strokeWidth={0.5}
                />
              </Link>
              ...
              {/* Relationship */}
              <Link
                href={isGuest ? '/auth/sign-up' : '/protected/family'}
                className="group relative h-64 bg-surface/40 border border-primary/20 rounded-2xl p-8 overflow-hidden hover:border-seal/60 transition-all hover:shadow-[0_0_30px_rgba(154,42,42,0.1)]"
              >
                <div className="relative z-10 flex flex-col h-full justify-between">
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                ...
                <Users
                  className="absolute -bottom-4 -right-4 w-48 h-48 text-seal/5 group-hover:text-seal/10 transition-colors -rotate-12"
                  strokeWidth={0.5}
                />
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
              ...
              <div className="col-span-12 md:col-span-4 space-y-4">
                <Link
                  href={isGuest ? '/auth/sign-up' : '/protected/ai-shaman'}
                  className="block bg-gradient-to-br from-surface to-background border border-white/5 rounded-2xl p-6 hover:border-primary/50 transition-colors group"
                >
                  ...
                  <h4 className="font-serif text-lg text-ink-light mb-1 group-hover:text-primary transition-colors">
                    해화당 AI
                  </h4>
                ...
                <Link
                  href={isGuest ? '/auth/sign-up' : '/protected/saju/today'}
                  className="block bg-gradient-to-br from-surface to-background border border-white/5 rounded-2xl p-6 hover:border-primary/50 transition-colors group"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="p-2 rounded-full bg-primary/20 text-primary">
                    ...
                    <Bell
                      className="w-4 h-4 text-ink-light/30 group-hover:text-primary transition-colors"
                      strokeWidth={1}
                    />
                  </div>
                  <h4 className="font-serif text-lg text-ink-light mb-1 group-hover:text-primary transition-colors">
              ...
              <div className="col-span-12 md:col-span-8">
                <div className="bg-surface/10 border border-white/5 rounded-2xl p-6 h-full">
                  <h3 className="text-xs font-bold text-ink-light/40 uppercase tracking-widest mb-6">
                    Discovery Tools
                  </h3>
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    {individualTools.map((tool, idx) => (
                      ...
                      >
                        <tool.icon
                          className="w-6 h-6 text-ink-light/40 mb-3 group-hover:text-primary transition-colors"
                          strokeWidth={0.8}
                        />
