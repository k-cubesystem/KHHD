...
export function CompatibilityMatrix({ nodes, edges }: CompatibilityMatrixProps) {
  ...
  const getTypeIcon = (type: CompatibilityEdge['type']) => {
    switch (type) {
      ...
      case 'friendship':
        return <Users className="w-3 h-3 text-primary-dark" />
    }
  }
            ...
            <svg ref={svgRef} width={width} height={height} className="mx-auto" viewBox={`0 0 ${width} ${height}`}>
              ...
              <g className="edges">
                {edges.map((edge) => {
                  ...
                  const isHighlighted =
                    selectedNode === edge.from || selectedNode === edge.to || hoveredEdge === `${edge.from}-${edge.to}`
                ...
                {nodes.map((node, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Relationship Badge */}
                      <text x={pos.x} y={pos.y + 50} textAnchor="middle" fontSize="11" fill="#989390">
                        {node.relationship}
                      </text>

                      {/* Connection Count */}
                      {relatedEdges.length > 0 && <circle cx={pos.x + 25} cy={pos.y - 25} r="12" fill="#C5B358" />}
