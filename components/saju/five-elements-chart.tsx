...
// Custom Tooltip — recharts 가 주입하는 프롭 중 실제로 쓰는 것만 선언한다(전부 optional).
interface ElementTooltipProps {
  active?: boolean
  payload?: Array<{ payload: ElementDatum }>
}

const CustomTooltip = ({ active, payload }: ElementTooltipProps) => {
  ...
  return (
    <Card className={cn('border-zen-border shadow-xl overflow-hidden bg-white relative', className)}>
      ...
      <CardHeader className="border-b border-zen-border bg-zen-bg/30">
        ...
        <p className="text-xs text-zen-muted mt-2">사주 팔자(八字)에서 오행의 분포를 시각화합니다</p>
      </CardHeader>
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart data={distribution}>
              ...
              <PolarAngleAxis dataKey="element" tick={{ fill: '#6B7280', fontSize: 14, fontFamily: 'Noto Serif KR' }} />
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h4 className="font-serif font-bold text-zen-text mb-1">용신(用神) 추천</h4>
              <p className="text-sm text-zen-muted leading-relaxed">
                당신의 사주에서 <span className="font-bold text-zen-wood">{recommendedElement.element}</span>의 기운이
                가장 부족합니다 ({recommendedElement.count}/8). 이 기운을 보완하면 운세의 균형을 맞출 수 있습니다.
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="text-xs px-2 py-1 bg-white rounded-sm border border-zen-border">
                  색상:{' '}
                  {recommendedElement.element === '목(木)'
                    ? '청색, 녹색'
                    : recommendedElement.element === '화(火)'
                      ? '적색, 주황'
                      : recommendedElement.element === '토(土)'
                        ? '황색, 갈색'
                        : recommendedElement.element === '금(金)'
                          ? '백색, 금색'
                          : '흑색, 청색'}
                </span>
                <span className="text-xs px-2 py-1 bg-white rounded-sm border border-zen-border">
