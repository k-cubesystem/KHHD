...
const TALISMAN_CONFIG = {
  fortune: {
    title: '길운부(吉運符)',
    color: GOLD_500,
    bgGradient: 'from-gold-300/30 via-gold-200/20 to-gold-300/30',
    symbol: '吉',
    description: '행운과 길한 기운을 불러오는 부적',
  },
  protection: {
    title: '호신부(護身符)',
    color: '#9E2B2B', // seal — 도장 레드
    bgGradient: 'from-seal/10 via-warning/10 to-seal/10',
    symbol: '護',
    description: '액운을 막고 몸을 보호하는 부적',
  },
  wealth: {
    title: '재물부(財物符)',
    color: '#4E9A6B', // bok-sprout
    bgGradient: 'from-bok-sprout/15 via-bok-sprout/10 to-bok-sprout/15',
    symbol: '財',
    description: '재물운과 사업운을 높이는 부적',
  },
  health: {
    title: '건강부(健康符)',
    color: '#2D5F8A', // obangsaek-blue
    bgGradient: 'from-obangsaek-blue/10 via-obangsaek-blue/5 to-obangsaek-blue/10',
    symbol: '健',
    description: '건강과 장수를 기원하는 부적',
  },
  love: {
    title: '인연부(因緣符)',
    color: '#C84040', // bok-flower — 연지 홍
    bgGradient: 'from-seal/10 via-seal/5 to-seal/10',
    symbol: '緣',
    description: '좋은 인연을 맺게 하는 부적',
  },
}

export function DigitalTalisman({ type = 'fortune', isRevealed = false, onReveal, className }: DigitalTalismanProps) {
  ...
  const handleReveal = () => {
    ...
    setTimeout(() => {
      confetti({
        ...
        spread: 70,
        origin: { y: 0.6 },
        colors: [config.color, '#D4AF37', '#C9A84C'],
      })
    }, 500)
    ...
    >
      <Card
        className={cn(
          ...
          revealed ? 'border-zen-gold shadow-2xl' : 'border-zen-border hover:border-zen-gold/50 hover:shadow-lg'
        )}
      ...
      >
        ...
        >
          {!revealed ? (
            // 미공개 상태
            <motion.div className="text-center space-y-4" whileHover={{ scale: 1.05 }}>
              <Scroll className="w-16 h-16 mx-auto text-zen-muted" />
              <p className="text-zen-muted font-serif">클릭하여 부적을 펼치세요</p>
            </motion.div>
          ) : (
            // 공개 상태
            <motion.div
            ...
            >
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <h3 className="text-2xl font-serif font-bold" style={{ color: config.color }}>
                    {config.title}
                  </h3>
                  <Sparkles className="w-5 h-5" style={{ color: config.color }} />
                </div>
                <p className="text-sm text-zen-muted">{config.description}</p>
              </div>
              ...
              >
                <div className="text-9xl font-serif font-black relative" style={{ color: config.color }}>
                  {config.symbol}
                  {/* 금빛 후광 */}
                  <div className="absolute inset-0 blur-2xl opacity-30" style={{ color: config.color }}>
                    {config.symbol}
                  </div>
                </div>
              </motion.div>
