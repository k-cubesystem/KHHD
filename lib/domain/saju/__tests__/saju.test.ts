...
describe('Saju Domain Logic', () => {
  describe('getSajuData', () => {
    ...
    it('should calculate elements distribution correctly', () => {
      ...
      expect(Object.keys(result.elementsDistribution)).toEqual(expect.arrayContaining(['木', '火', '土', '金', '水']))

      const total = Object.values(result.elementsDistribution).reduce((sum, val) => sum + val, 0)
  ...
  describe('calculateDaeun', () => {
    it('should return 10 daeun periods with correct start age (기운 나이)', () => {
      const result = calculateDaeun('1990-05-15', '14:30', 'M', true)

      expect(result).toHaveLength(10)
      // 1990-05-15 14:30 남성: 庚午년(양간) 순행 → 입하 절기 거리 기반 7세 기운
      // (lunar-javascript getYun 검증값: 1997년 시작)
      expect(result[0].age).toBe(7)
      expect(result[9].age).toBe(97)
    })
