...
import { SajuData, toLunar } from './saju'
...
export function analyzeGekguk(saju: SajuData): GekgukAnalysis {
  ...
  let hanja = '正官格'
  let description =
    '월지에 정관이 투출하여 정관격을 이룹니다. 정관격은 명예와 지위를 중시하며, 책임감이 강한 성향을 나타냅니다.'
  let characteristics = [
  ...
  // 오행 분포에 따른 격국 힌트
  const dominantElement = Object.entries(elementsDistribution).reduce((a, b) => (a[1] > b[1] ? a : b))[0]
...
export function calculateSinsal(saju: SajuData): SinsalItem[] {
  ...
  if (pillarZhis.includes(yeokma)) {
    sinsalList.push({
      ...
      influence:
        '해외 활동이나 출장이 많을 수 있으며, 직업이나 거주지가 자주 바뀔 수 있습니다. 여행업, 무역, 운송업 등에 적합합니다.',
    })
  }
  ...
  if (cheonul.some((z: string) => cheoneulPillarZhis.includes(z))) {
    sinsalList.push({
      ...
      influence:
        '위기 상황에서 예상치 못한 도움을 받을 수 있으며, 인복이 좋습니다. 윗사람이나 권력자의 도움을 받기 쉽습니다.',
    })
  }
  ...
  if (hwagaePillarZhis.includes(hwagae)) {
    sinsalList.push({
      ...
      influence:
        '혼자만의 시간을 즐기며 창작 활동에 적합합니다. 예술, 종교, 학문, 철학 분야에서 재능을 발휘할 수 있습니다.',
    })
  }
  ...
  if (dohwaPillarZhis.includes(dohwa)) {
    sinsalList.push({
      ...
      influence:
        '대인관계가 원활하고 사람들의 주목을 받기 쉽습니다. 연예, 서비스, 영업 분야에 유리하나, 이성 관계에 주의가 필요합니다.',
    })
  }
...
export function calculateDaeun(
  ...
  gender: string,
  _saju: SajuData,
  birthTime: string = '00:00',
  isSolar: boolean = true
): DaeunPeriod[] {
  try {
    // 기운(起運) 나이·순행/역행은 lunar-javascript getYun()이 절기 거리 기반으로 계산
    const lunar = toLunar(birthDate, birthTime, isSolar)
    const eightChar = lunar.getEightChar()
    eightChar.setSect(1)

    const birthYear = lunar.getSolar().getYear()
    const yun = eightChar.getYun(gender === 'female' ? 0 : 1)

    // index 0은 대운 시작 전(원국) 구간이므로 제외하고 8개 대운 사용
    return yun
      .getDaYun(9)
      .slice(1)
      .map((daYun) => {
