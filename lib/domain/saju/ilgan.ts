/**
 * 일간(日干) 10종 — 일간 데이터의 단일 출처.
 *
 * 🔴 2026-09-08: 「3초 일간」 공개 페이지(/ilgan)는 「3초 사주」(/saju3)로 개조되며 사라졌지만
 *    (next.config 308 리다이렉트), **이 데이터 모듈은 남는다.** 그 사이 다른 갈래에서 소비처가
 *    둘 붙었기 때문이다 — 사주 공유카드 OG(app/api/og/saju-card/[stem])와 의례 폴백 문장
 *    (lib/domain/ritual/fallback-line.ts). 브랜치 병합 때 페이지와 함께 지우면 둘 다 빌드가 깨진다.
 *
