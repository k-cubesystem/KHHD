  ...
  const remedyTags =
    remedyCount > 0
      ? Array.from({ length: remedyCount }, (_, i) =>
          i === 0
            ? '[[REMEDY_1: 아래 [개운 처방] 블록의 **첫 번째 항목**을 풀어 쓴다 — 무엇을 하는지 → 왜 이 사주에 그것인지 → 언제 하면 되는지 (9~12문장 · 최소 450자)]]'
            : `[[REMEDY_${i + 1}: ${i + 1}번째 항목 (9~12문장 · 최소 450자)]]`
        ).join('\n')
      : ''
...
[[NOW: 현재 구간을 쉬운 비유 한 구절로 (예: 오르막을 거의 다 올라온 자리) | 대운×삼정×손금을 겹쳐 본 현재 국면 — 어디를 보고 그렇게 읽었는지 하나하나 짚으며 (18~24문장 · **최소 1,100자**)]]
[[NATURE: 타고난 그릇을 비유 한 구절로 (예: ${sajuBlock.dayMasterPoetic || '우뚝 선 소나무'}) | 어떤 사람으로 태어났는지 — 일간 비유로 시작해 성격의 결·강점·조심할 결까지, 얼굴과 손이 그 그릇을 어떻게 보여주는지 함께 (21~30문장 · **최소 1,300자**)]]
[[CROSS_WEALTH: 합|반합|충 중 하나 | 재물 — 사주 재성·코(재백궁)·재물선을 하나하나 짚으며 엮은 해석. 돈이 들어오는 모양·새는 구멍·키우는 법 (18~27문장 · **최소 1,100자**)]]
[[CROSS_CAREER: 판정 | 일·명예 — 관성·이마(관록궁)·운명선을 엮은 해석. 조직/독립 어느 쪽 그릇인지 포함 (18~27문장 · **최소 1,100자**)]]
[[CROSS_LOVE: 판정 | 인연 — 배우자성·눈꼬리(처첩궁)·감정선을 엮은 해석. 사랑할 때의 모습과 필요한 온도 (18~27문장 · **최소 1,100자**)]]
[[CROSS_HEALTH: 판정 | 건강·바탕 — 일간 강약·기색·생명선을 엮은 해석. 지치는 패턴과 회복 스위치 (18~27문장 · **최소 1,100자**)]]
[[MATCH_PEOPLE: 잘 맞는 사람을 한 줄로 | 나랑 잘 맞는 사람 — 위 궁합 힌트를 바탕으로: 어떤 기운·성격의 사람이 힘이 되는지, 그런 사람은 인상이 어떤지(얼굴 비유), 반대로 기 빨리기 쉬운 유형과 거리 조절법, 배우자성·연애 패턴까지 (24~36문장 · **최소 1,400자**)]]
[[MATCH_JOB: 잘 맞는 일을 한 줄로 | 나랑 잘 맞는 직업 — 위 적성 힌트를 바탕으로: 구체 직업군 예시를 여러 개 들고 왜 맞는지(사주 구조·이마·두뇌선·운명선 근거), 피하고 싶을 환경, 지금 하는 일과 다를 때의 현실적 활용법 (24~36문장 · **최소 1,400자**)]]
