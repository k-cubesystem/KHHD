 ...
 *
 * ## 🔴 장 수를 라벨에 쓰지 않는다
 * 재물운·종합풀이는 이용권을 쓰는 화면이지만, 런처는 «어디로 가는지»만 말한다. 값은 도착 화면이
 * 실사용 키(`feature-costs.ts`)에서 파생해 밝힌다 — 표시 = 실사용 단일 출처를 깨지 않는다.
 */
 ...
 * 첫 칸이 사주풀이인 것은 복주머니 카드의 첫 주머니와 같은 목적지라 일부러다 — 아이콘으로 한 번,
 * 배너로 한 번, 같은 문을 두 크기로 연다. (구 사주 유도 카드 `MasterpieceSection` 은 2026-08-24
 * 배너 일원화로 삭제됐다 — 그 카피는 `journey-card.tsx` 가 들고 있다.)
 *
