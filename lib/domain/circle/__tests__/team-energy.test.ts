...
describe('circleRoles — 십성 다섯 무리 합산', () => {
  it('두꺼운 결·옅은 결을 그룹 단위로 합쳐 말하고, 분포가 없으면 null', () => {
    ...
    expect(roles?.thick.people).toBe('아이디어를 내고 만들어 내는 사람')
    expect(roles?.sentence).toContain('아이디어를 내고 만들어 내는 사람이 많고')
    expect(roles?.sentence).toContain('일을 벌이고 열매를 거두는 사람은 적습니다')
