/**
 * 팀 기운 지도 — 무리 한 벌의 «전체 균형 · 그 기운을 든 사람 · 서로의 관계 · 역할 결».
 *
 ...
 *    직장 무리에서 숫자가 생길 여지를 구조적으로 없애는 것이다(PRD-energy-circle §3-6).
 */
...
/** 무리의 한 사람 — 기운(지도와 같은 값) + 명식 힌트(없으면 null). */
...
/** 그 기운을 «든» 사람 — 넘치는 기운이 그것이고, 무리 평균보다 넉넉할 때. */
...
/** 모든 짝 — 입력 순서대로(i<j). 정렬을 바꾸지 않아 같은 무리는 늘 같은 순서다. */
...
export function circleRoles(entries: readonly CircleMemberEnergy[]): CircleRoles | null {
  ...
  return {
    ...
    sentence: `이 무리는 ${thick.plain}이 두껍고, ${thin.plain}이 옅습니다. 옅은 결은 지금 있는 사람 중 그 결에 가까운 사람과, 그 결을 보태는 물건으로 채웁니다.`,
  }
}
