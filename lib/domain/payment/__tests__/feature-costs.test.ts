 ...
 *  - 사주·궁합·신년·오늘 = 차감 없음(무료) — 해당 액션에 deductTalisman 미존재 확인
...
describe('FEATURE_COST — 표시 = 실차감 단일 소스', () => {
  ...
  it('무료 기능은 0 이며 free 플래그가 참이다', () => {
    for (const key of ['saju', 'compatibility', 'newYear', 'today'] as const) {
  ...
  it('유료 기능은 free 플래그가 거짓이다', () => {
    for (const key of ['face', 'palm', 'fengshui', 'wealth', 'imageGeneration'] as const) {
  ...
  it('MISSION 카테고리 → 비용 매핑이 단일 소스를 따른다', () => {
    ...
    expect(featureCostByCategory('SAJU')).toBe(0)
    expect(featureCostByCategory('COMPATIBILITY')).toBe(0)
    expect(featureCostByCategory('NEW_YEAR')).toBe(0)
    expect(featureCostByCategory('TODAY')).toBe(0)
    expect(isFreeFeatureCategory('SAJU')).toBe(true)
    expect(isFreeFeatureCategory('FACE')).toBe(false)
  })

  it('표시 문자열은 "무료" 또는 "복채 N만냥" 형태로 통일된다 ("N복채" 금지)', () => {
    expect(formatFeatureCost('saju')).toBe('무료')
