 ...
 *
 * 핵심 계약:
 *  1. 전액 취소는 발급 전량을 회수한다(반올림 누수 없음).
...
describe('computeCancelClawback — 전액 취소', () => {
  ...
  it('부분 취소가 쌓여 남은 결제 금액이 0 이 되면 남은 발급분을 쓸어 담는다', () => {
    // 40% 를 이미 회수(8장)한 상태에서 나머지가 취소됨 — 비율 반올림으로 1장이 남으면 안 된다.
    const plan = computeCancelClawback({
      paidAmount: 10_000,
      creditsGranted: 20,
      creditsRemaining: 12,
...
describe('computeCancelClawback — 부분 취소', () => {
  ...
  it('2차 부분 취소는 누적 목표에서 이미 회수분을 뺀 증분만 회수한다', () => {
    // 1차로 3,000원(6장) 회수 완료 → 2차 2,000원 추가 취소 → 누적 5,000원(10장) 목표
    const plan = computeCancelClawback({
      paidAmount: 10_000,
      creditsGranted: 20,
      creditsRemaining: 14,
  ...
  it('금액이 아니라 발급 장 수 기준으로 비례 회수한다', () => {
    // 1만원 결제에 50장 발급 → 절반 취소면 25장 회수
    const plan = computeCancelClawback({
      paidAmount: 10_000,
      creditsGranted: 50,
      creditsRemaining: 50,
...
describe('computeCancelClawback — 오회수 방지', () => {
  ...
  it('취소 금액이 결제 금액을 넘어도 발급 총량을 넘겨 회수하지 않는다', () => {
    const plan = computeCancelClawback({
      ...basePayment(),
      tossStatus: 'PARTIAL_CANCELED',
