 ...
 *
 * 왜 필요한가
 *  이용권을 이미 써버린 뒤에도 2차 동의를 거치면 취소가 되고, 회수하지 못한 몫은 회사가 손실로
 *  떠안는다(CEO 결정 — 사용자에게 청구하지 않는다). 이 경로는 구조적으로
 *  「구매 → 이용권 전부 사용 → 전액 환불」을 반복할 수 있고, 지금까지 있던 방어는 **결제 1건당 1회**
 ...
 *
 * 수치 근거
 *  - 복채 시절(충전 팩 5만~30만원)에 정한 값이다. 2026-09-18 이용권 팩은 4,800·19,800·38,800원이라
 *    지금은 **횟수(연 2회)가 먼저 닫히고**, 금액(10만원)은 큰 팩을 여러 번 돌리는 경우의 두 번째 벽이다.
 *    1회 결제 상한도 10만원(토스)이라 단일 손실은 그 안에서 유계다.
 *  - 횟수만 두면 큰 팩을 두 번 돌려 새고, 금액만 두면 소액 반복이 상한 직전까지
 *    계속 성공한다. 그래서 둘 다 둔다.
 *  - 달력 연도가 아니라 이동창인 이유: 12월 31일과 1월 1일에 연달아 뽑는 연초 리셋 악용이 있다.
 *
 * 🔴 판정은 «이미 쌓인 누적»만 본다 — 이번 요청의 예상 손실을 더해서 넘는지 보지 **않는다**.
 *    사전 합산으로 막으면 큰 팩을 쓴 정상 사용자가 **첫 요청부터** 거절당한다(구제 자체가 사라짐).
 *    이 방식이면 첫 요청은 팩 크기와 무관하게 반드시 통과하고, 큰 손실이 한 번 나면 그 즉시
 *    금액 상한이 닫혀 두 번째가 막힌다. 연간 최대 노출 = 「단일 최대 손실 1회」로 유계다.
 *
 * 🔴 손실이 0인 취소(이용권을 안 쓴 정상 취소)는 이 상한과 **무관**하다. lossCredits > 0 일 때만 잰다.
 */
...
export interface LossCapInput {
  /** 이번 취소로 손실 처리될 이용권 장 수. 0 이면 상한과 무관하다 */
  lossCredits: number
  usage: LossCapUsage
...
export function lossCapBlockedMessage(decision: LossCapDecision): string {
  ...
  if (decision.blockedReason === 'AMOUNT_EXCEEDED') {
    return (
      '이미 사용하신 이용권까지 되돌려 드리는 취소는 한 계정에 1년 동안 도와드릴 수 있는 한도가 있습니다. ' +
      `최근 1년 사이 그 한도에 이르러 지금은 자동으로 처리해 드리기 어렵습니다.${reopen}${guide}`
    )
  }
  return (
    '이미 사용하신 이용권까지 되돌려 드리는 취소는 한 계정에 1년 2회까지 도와드리고 있습니다. ' +
