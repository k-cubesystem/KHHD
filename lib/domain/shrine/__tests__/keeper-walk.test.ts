...
const ENTRANCE = { from: 8, to: 45, ms: 1800 }
...
describe('planKeeperWalk — 배회 구간', () => {
  ...
  it('한 바퀴는 구간을 두 번 훑는 길이 — 「구간 1회 24s」와 같은 걸음 속도', () => {
...
describe('planKeeperEntrance — 입장 걷기 구간', () => {
  it('배선 값(문간 8 → 제단 옆 45)이 그대로 통과한다', () => {
    expect(planKeeperEntrance(ENTRANCE, planKeeperWalk(WANDER))).toEqual({ from: 8, to: 45, ms: 1800 })
  })
