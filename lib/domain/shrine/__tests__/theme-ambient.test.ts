...
describe('THEME_AMBIENT — tintProfile 확정값', () => {
  it('amp 는 0~1', () => {
    for (const code of THEMES) {
      const { amp } = THEME_AMBIENT[code].tintProfile
      expect(amp).toBeGreaterThan(0)
      expect(amp).toBeLessThanOrEqual(1)
    }
  })

  it('밤 원판 테마는 달집·별밭·연등·도깨비 넷뿐(amp 0.4)', () => {
    ...
    for (const c of night) expect(THEME_AMBIENT[c].tintProfile.amp).toBe(0.4)
  })

  it('홍살=노을 원판(dusk 0.5) · 용궁=무시간 심해(null 0.3)', () => {
    expect(THEME_AMBIENT.hongsal.tintProfile).toEqual({ base: 'dusk', amp: 0.5 })
    expect(THEME_AMBIENT.yonggung.tintProfile).toEqual({ base: null, amp: 0.3 })
  })

  it('나머지 10테마는 낮 원판(amp 1)', () => {
    const day = THEMES.filter((c) => THEME_AMBIENT[c].tintProfile.base === 'day')
    expect(day).toHaveLength(10)
    for (const c of day) expect(THEME_AMBIENT[c].tintProfile.amp).toBe(1)
  })
})
...
describe('tintOpacities — 위상 틴트 opacity', () => {
  ...
  const DUSK: TintProfile = { base: 'dusk', amp: 0.5 }
  const TIMELESS: TintProfile = { base: null, amp: 0.3 }

  it('낮 순수 위상에서는 전 테마가 셋 다 0 — 검수된 원화가 그대로 보인다', () => {
    for (const code of THEMES) {
      expect(tintOpacities(PURE_HOUR.day, THEME_AMBIENT[code].tintProfile)).toEqual({ dawn: 0, dusk: 0, night: 0 })
    }
    // 블렌드 창 밖의 낮 시간대 전체가 0 이어야 한다(9~16시)
    for (let h = 9; h <= 16; h += 0.25) {
      expect(tintOpacities(h, DAY)).toEqual({ dawn: 0, dusk: 0, night: 0 })
    }
  })

  it('낮 원판 테마: 순수 위상에서 그 위상 성분만 1', () => {
    expect(tintOpacities(PURE_HOUR.night, DAY)).toEqual({ dawn: 0, dusk: 0, night: 1 })
    expect(tintOpacities(PURE_HOUR.dawn, DAY)).toEqual({ dawn: 1, dusk: 0, night: 0 })
    expect(tintOpacities(PURE_HOUR.dusk, DAY)).toEqual({ dawn: 0, dusk: 1, night: 0 })
  })

  it('밤 원판 테마: 밤에 night 성분 0 — 밤 그림에 밤 틴트를 겹치지 않는다', () => {
    ...
    for (const code of ['daljip', 'byeolbat', 'yeondeung', 'dokkaebi']) {
      ...
      for (const h of [0, 2, 4, 20, 21, 22, 23.5]) expect(tintOpacities(h, profile).night).toBe(0)
    }
  })

  it('밤 원판 테마의 dawn·dusk 는 살아 있되 amp 만큼만', () => {
    expect(tintOpacities(PURE_HOUR.dawn, NIGHT)).toEqual({ dawn: 0.4, dusk: 0, night: 0 })
    expect(tintOpacities(PURE_HOUR.dusk, NIGHT)).toEqual({ dawn: 0, dusk: 0.4, night: 0 })
  })

  it('노을 원판 테마(홍살): 석양에 dusk 0, 밤은 amp 만큼', () => {
    expect(tintOpacities(PURE_HOUR.dusk, DUSK).dusk).toBe(0)
    expect(tintOpacities(PURE_HOUR.night, DUSK)).toEqual({ dawn: 0, dusk: 0, night: 0.5 })
    for (let h = 17; h <= 19; h += 0.1) expect(tintOpacities(h, DUSK).dusk).toBe(0)
  })

  it('무시간 원판(용궁): 세 위상 모두 살아 있고 진폭만 amp', () => {
    expect(tintOpacities(PURE_HOUR.night, TIMELESS)).toEqual({ dawn: 0, dusk: 0, night: 0.3 })
    expect(tintOpacities(PURE_HOUR.dawn, TIMELESS).dawn).toBe(0.3)
    expect(tintOpacities(PURE_HOUR.dusk, TIMELESS).dusk).toBe(0.3)
  })

  it('amp 스케일 — 같은 시각에서 amp 에 정비례', () => {
    for (const h of [0, 6.5, 18.5, 4.5, 19.4]) {
      const full = tintOpacities(h, { base: 'day', amp: 1 })
      const half = tintOpacities(h, { base: 'day', amp: 0.5 })
    ...
    expect(tintOpacities(PURE_HOUR.night, { base: 'day', amp: 0 })).toEqual({ dawn: 0, dusk: 0, night: 0 })
  })
  ...
  it('전 구간·전 테마에서 0~1 범위, 합도 amp 이하', () => {
    for (const code of THEMES) {
      const profile = THEME_AMBIENT[code].tintProfile
      for (let i = 0; i <= 2400; i += 7) {
        const o = tintOpacities(i / 100, profile)
        const sum = o.dawn + o.dusk + o.night
        for (const v of [o.dawn, o.dusk, o.night]) {
          expect(v).toBeGreaterThanOrEqual(0)
          expect(v).toBeLessThanOrEqual(1)
        }
        expect(sum).toBeLessThanOrEqual(profile.amp + 1e-6)
      }
    }
  })
  ...
  it('scene-clock 위상 판정을 그대로 쓴다 — 가중치와 어긋나지 않는다', () => {
    for (const h of [0, 4.5, 5, 6, 7.5, 8, 12, 16.5, 17, 19, 20, 21]) {
      ...
      const o = tintOpacities(h, { base: 'day', amp: 1 })
  ...
  it('쓰레기 입력 방어 — 유한값만 나간다', () => {
    ...
    expect(tintOpacities(PURE_HOUR.night, { base: 'day', amp: Number.NaN })).toEqual({ dawn: 0, dusk: 0, night: 0 })
    expect(tintOpacities(PURE_HOUR.night, { base: 'day', amp: 9 })).toEqual({ dawn: 0, dusk: 0, night: 1 })
    expect(tintOpacities(PURE_HOUR.night, { base: 'day', amp: -3 })).toEqual({ dawn: 0, dusk: 0, night: 0 })
  })
...
describe('ambientForTier — 티어 적용', () => {
  ...
  it('없는 층은 키 자체를 만들지 않는다 (undefined 키 유입 금지)', () => {
    const bare: ThemeAmbient = { tintProfile: { base: 'day', amp: 1 } }
