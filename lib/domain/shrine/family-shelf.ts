...
export const FSHELF_UNIT = Object.freeze({
  ...
  top: 20,
  /**
   * 하단 y — **동결값**이다. v3 마루선(60)에 발을 2%p 묻고 서던 자리인데, 무대 기하 v4 가
   * 마루선을 v4 70 · v4.1 **73** 으로 내리면서 그 관계가 끊겼다(밑동이 마루선보다 11%p 위에 뜬다).
   * 그래도 마루선 파생으로 바꾸지 않는다 — 상자가 커지면 진열 칸 앵커가 함께 내려가
   * **이미 진열한 아이템이 널에서 떨어지고**(배치는 절대 좌표), 스프라이트도 세로로 늘어난다.
   * 뜬 만큼은 신당별 조절(fixtureOffsets.familyShelf.dy)이 덮는다.
   * (계약: theme-stage.test 「사방탁자 밑동 62 는 동결이다」)
   */
  bottom: 62,
})
