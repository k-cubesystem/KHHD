/**
 * 속풀이 문답 파이프라인 — 서버 액션(비스트리밍)과 SSE 라우트(스트리밍)가 **함께 쓰는** 코어.
 *
 * 왜 파일을 갈랐나: 서버 액션은 스트림을 돌려줄 수 없어 스트리밍은 라우트 핸들러가 져야 한다.
 * 그런데 조립(인증·차감·명식 컨텍스트·페르소나)과 마무리(감정·인연·잔여)는 두 경로가 같아야 한다 —
 * 갈라 두면 한쪽만 고쳐지고 다른 쪽이 조용히 뒤처진다(엣지 ai-chat 이 그렇게 죽었다).
 *
