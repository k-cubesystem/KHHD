...
export function getAnalysisTypeGuide(type: AnalysisType): string {
  const guides: Record<AnalysisType, string> = {
    SAJU_FULL: `
[종합 사주 분석 지침 — "과거->현재->미래" 3단계 신뢰 공식]

★ 최우선: 과거 역추산 (pastRetrograde)
- 위 '과거 대운 상세' 데이터를 반드시 참조하십시오
- 과거 대운 천간이 일간을 충(衝)하거나 형(刑)하는 시점을 찾으십시오
- 해당 시기에 발생했을 사건을 "직업 변화/이별/건강 이슈/이사/새로운 시작" 카테고리로 추론하십시오
- 세운(해당 연도의 간지)과 원국의 충/합도 교차 분석하십시오
- 반드시 3개 이상의 과거 시점을 제시하십시오

★ 두 번째: 현재 공감 (currentSituation)
- 현재 대운과 2026년 세운(병오년)이 만들어내는 에너지를 분석하십시오
- "지금 ~하고 계시죠" 형태의 2인칭 공감 표현을 사용하십시오
- 즉시 실행 가능한 행동 조언을 시기와 함께 제시하십시오

★ 세 번째: 미래 처방
- 월별 운세를 "7월 셋째 주" 수준의 구체적 시기로 좁히십시오
- 모든 처방에 실행 가능한 행동을 포함하십시오 ("빨간색 착용", "서쪽 방향 피하기")
