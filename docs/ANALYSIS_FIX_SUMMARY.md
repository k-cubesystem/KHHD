...
const { data: target } = await supabase.from('v_destiny_targets').select('*').eq('id', targetId).single()
```
...
if (target.target_type === 'self') {
  // 본인(profiles) 업데이트
  await supabase.from('profiles').update({ avatar_url: faceImageUrl }).eq('id', targetId)
} else {
