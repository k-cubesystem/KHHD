...
/**
 * 이용권 주문 확인 화면의 결제 버튼 — 일반결제(카드) 한 번.
 *
 * 🔴 결제창을 여는 값(금액·상품명·장 수)은 **서버가 DB 에서 읽어 내려준 것**만 쓴다.
 *    이 컴포넌트가 가격을 계산하면 브라우저에서 바꿔 부를 수 있다. 승인 쪽(confirmPayment)도
 *    장 수로 상품을 다시 찾아 금액을 대조한다.
 * 🔴 동의 전에는 버튼이 잠긴다 — 체크를 받고도 결제가 되면 받는 의미가 없다.
 */
export function PassCheckoutClient({
  memberId,
  orderName,
  amount,
  passes,
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const orderId = `PASS_${Date.now()}_${memberId.slice(0, 6)}`
      ...
      await payment.requestPayment({
        ...
        orderId,
        orderName,
        successUrl: `${window.location.origin}/protected/analysis/success?memberId=${memberId}&passes=${passes}`,
