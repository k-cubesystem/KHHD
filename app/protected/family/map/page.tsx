...
export default async function FamilyEnergyMapPage({ searchParams }: { searchParams: Promise<{ circle?: string }> }) {
  ...
  // 내가 만든 그룹(직장·모임·직접 이름) — 가족 지도와 다른 화면이다(고지·밴드 모드).
  if (circle && circle !== FAMILY_CIRCLE_ID) {
    const [payload, membership, narrative] = await Promise.all([
      getCircleEnergy(circle),
      getCurrentUserMembership(),
      getCachedNarrative('circle', circle),
    ])
    if (!payload) {
      return (
        <div className="min-h-screen w-full max-w-[480px] mx-auto px-4 py-16">
          <div className="text-center space-y-5 border border-dashed border-gold-500/20 bg-surface/20 rounded-xl p-10">
            ...
            <h1 className="text-xl font-serif font-bold text-ink-light">그 그룹을 찾지 못했습니다</h1>
            <p className="text-sm text-ink-light/55 leading-relaxed">지워졌거나 내 그룹이 아닙니다.</p>
            <Link
  ...
  const [data, family, narrative] = await Promise.all([
    getFamilyEnergyMap(),
    getCircleEnergy(FAMILY_CIRCLE_ID),
    getCachedNarrative('circle', FAMILY_CIRCLE_ID),
  ])
  ...
  return <FamilyEnergyMapView data={data} teamEntries={family?.energy.entries ?? []} narrative={narrative} />
}
