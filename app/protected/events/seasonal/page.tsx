      ...
      <div className="px-4 pt-6 max-w-md mx-auto space-y-6">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gold-500/10 border border-gold-500/20">
            ...
            <span className="text-[10px] font-bold text-gold-500 tracking-widest uppercase">24절기 운세 달력</span>
          </div>
          ...
          {displayEvent ? (
            ...
            <>
              ...
              <h1 className="text-2xl font-serif font-bold text-white/80">24절기 운세 달력</h1>
              <p className="text-white/50 text-sm">지금은 달력에 없는 기간이에요.</p>
            </>
          )}
        </motion.header>
        ...
        {displayEvent && (
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-white/50 mb-0.5">지금 절기</p>
                <p className="text-3xl font-serif font-bold text-gold-500">{displayEvent.hanja}</p>
              </div>
              {timeLeft && (
                <div className="text-right">
                  <p className="text-xs text-white/40 mb-1 flex items-center gap-1 justify-end">
                    <Clock className="w-3 h-3" /> 다음 절기까지
                  </p>
                  ...
                  : displayEvent.element === '화'
                    ...
                    : displayEvent.element === '토'
                      ...
                      : displayEvent.element === '금'
                        ? '金'
                        : '水'}
                ) 기운이 강한 시기예요. 이번 절기에는 «{displayEvent.fortuneType}»을 눈여겨보세요.
              </p>
            </div>
          </motion.div>
        )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
            <div className="space-y-2">
              ...
              <p className="text-center text-xs text-white/30">
                이번 절기는 {formatDate(displayEvent.endMonth, displayEvent.endDay)}까지예요.
              </p>
            </div>
          </motion.div>
        )}
        ...
        {activeTab === 'current' && !displayEvent && upcoming && (
          ...
          >
            ...
            <p className="text-sm text-white/60">다음 절기</p>
            <p className="text-lg font-serif font-bold text-gold-500">
              {upcoming.name}({upcoming.hanja})
            </p>
            <p className="text-xs text-white/40">
              {formatDate(upcoming.startMonth, upcoming.startDay)}부터 · {upcoming.fortuneType}
            </p>
          </motion.div>
        )}
        ...
        {activeTab === 'calendar' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-bold text-white/80">2026년 24절기 일정</span>
            </div>
