  ...
  if (!membership && status.totalRemaining <= 0) {
    return (
      <MembershipGate
        feature="counsel"
        title="속풀이 · 신령님과의 문답"
        description="좌정하신 神이 사주를 근거로 답합니다. 작은 질문도 좋습니다 — 광고를 보고 한 번 여쭙거나, 이용권으로 질문을 열어 이어가세요."
        benefits={[
          `멤버십 회원은 주 ${MEMBER_WEEKLY_QUESTIONS}문`,
          `명식을 채우면 맛보기 ${ONBOARDING_FREE_QUESTIONS}문`,
          `${formatFeatureCost('shamanQuestions')}으로 질문 ${PURCHASE_QUESTIONS}문 · ${PURCHASE_EXPIRE_DAYS}일 이내 사용`,
          '신당 · 가족관리 입장 포함',
