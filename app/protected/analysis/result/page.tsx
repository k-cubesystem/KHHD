...
export default function AnalysisResultPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-background text-white font-sans flex flex-col relative overflow-hidden pb-24">
      ...
      {/* Header */}
      <header className="relative z-20 px-3 pt-12 pb-4 flex items-center justify-between">
        <Link href="/protected" className="p-2 -ml-2 text-white/60 hover:text-white transition-colors">
          <ChevronLeft className="w-6 h-6" />
        </Link>
        <span className="font-serif font-semibold text-primary tracking-widest gold-glow text-lg">운명 분석</span>
        <div className="w-10" /> {/* Spacer */}
      </header>

      {/* Main Content */}
      <div className="relative z-10 px-3 flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        {/* Section 1: The Four Pillars (Saju) */}
        <section className="space-y-6">
          <div className="flex items-center gap-4 justify-center">
            <div className="h-px w-8 bg-primary/30" />
            <h2 className="text-center text-xs font-serif font-bold tracking-[0.2em] text-primary/80 uppercase">
              사주 팔자
            </h2>
            <div className="h-px w-8 bg-primary/30" />
          </div>
              ...
              return (
                <div key={idx} className="flex flex-col gap-2 items-center">
                  ...
                  <div className="w-full aspect-[1/2] border border-primary/20 bg-surface/40 backdrop-blur-sm rounded-lg flex flex-col items-center justify-center gap-4 relative overflow-hidden luxury-card-glow transition-transform active:scale-95 duration-300">
                    <span className={`font-serif text-2xl font-bold ${pillar.color}`}>{pillar.gan}</span>
                    <div className="w-4 h-px bg-white/10" />
                    <span className={`font-serif text-2xl font-bold text-white/90`}>{pillar.ji}</span>

                    {/* Elemental Aura */}
                    <div
                      className={`absolute -bottom-4 -right-4 w-16 h-16 blur-[40px] opacity-10 ${pillar.color.replace('text-', 'bg-')}`}
                    />
                  </div>
                </div>
              )
        ...
        <section className="space-y-6">
          ...
          <div className="bg-surface/30 border border-primary/10 rounded-xl p-6 luxury-card-glow relative overflow-hidden">
            ...
            <div className="flex justify-between px-2 mt-2">
              {fiveElements.map((el, i) => (
                <span key={i} className="text-[10px] text-white/30 tracking-wider w-full text-center">
                  {el.label.split(' ')[0]}
                </span>
              ))}
            </div>
        ...
        <section className="space-y-4">
          <div className="bg-surface/30 border-l-2 border-primary/50 p-6 rounded-r-xl luxury-card-glow">
            ...
            <p className="font-sans text-base text-white/80 leading-relaxed font-light">
              ...
              이는 폭발적인 추진력과 열정을 의미합니다. 다만 <span className="text-blue-400 font-medium">수(水)</span>가
              부족하여 마무리가 약할 수 있으니 인내심을 기르는 것이 중요합니다.
            </p>
          </div>
          ...
          <div className="bg-surface/30 border-l-2 border-seal/50 p-6 rounded-r-xl luxury-card-glow">
            ...
            <p className="font-sans text-base text-white/80 leading-relaxed font-light">
              올해는 무리한 확장보다는{' '}
              <span className="text-white font-medium border-b border-white/20 pb-0.5">내실을 다지는 것</span>이
              유리합니다. 북쪽 방향이 길하며, 검은색 소품이 행운을 가져다 줄 것입니다.
            </p>
          </div>
        </section>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-background via-background/95 to-transparent z-50">
        ...
        <div className="max-w-[480px] mx-auto mt-3 text-center">
          <Link
            href="/protected/saju/new"
            className="text-xs text-white/40 hover:text-white flex items-center justify-center gap-1.5 transition-colors py-2"
          >
