...
export function WealthAnalysisContent() {
    ...
    if (isGuest) {
        return (
            <div className="flex flex-col gap-12 w-full max-w-5xl mx-auto py-12 px-6 pb-24">
                <GuestCTACard
    ...
    return (
        <motion.div
            ...
            initial="initial"
            animate="animate"
            className="flex flex-col gap-12 w-full max-w-5xl mx-auto py-12 px-6 pb-24"
        >
                        ...
                        <CardContent className="p-8">
                            ...
                            >
                                {wealthAnalysis.split('\n').map((line, index) => {
                                    if (line.startsWith('# ')) {
                                        return <h1 key={index} className="text-3xl font-serif font-bold text-ink-light mb-6">{line.substring(2)}</h1>;
                                    } else if (line.startsWith('## ')) {
                                        return <h2 key={index} className="text-2xl font-serif font-bold text-primary mt-8 mb-4">{line.substring(3)}</h2>;
                                    } else if (line.startsWith('### ')) {
                                        return <h3 key={index} className="text-xl font-serif font-bold text-ink-light mt-6 mb-3">{line.substring(4)}</h3>;
                                    } else if (line.startsWith('**') && line.endsWith('**')) {
                                        return <p key={index} className="font-bold text-primary my-3">{line.substring(2, line.length - 2)}</p>;
                                    } else if (line.startsWith('- ')) {
                                        return <li key={index} className="text-ink-light/80 ml-4">{line.substring(2)}</li>;
                                    } else if (line.startsWith('---')) {
                                        return <hr key={index} className="my-6 border-primary/20" />;
                                    } else if (line.trim()) {
                                        return <p key={index} className="text-ink-light/80 leading-relaxed my-2">{line}</p>;
                                    }
                                    return <br key={index} />;
                                })}
                            </div>
            ...
            {!wealthAnalysis && !analyzing && (
                <motion.div variants={fadeInUp} className="text-center py-12">
                    ...
                    <p className="text-ink-light/50 font-serif">
                        위의 버튼을 눌러 재물운 분석을 시작하세요
                    </p>
                </motion.div>
            )}
        </motion.div>
