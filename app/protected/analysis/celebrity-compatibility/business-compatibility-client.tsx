...
import { Briefcase, ArrowLeft, Loader2, Users, TrendingUp, AlertTriangle, Sparkles, Target, Building2 } from 'lucide-react'
...
function getScoreLabel(score: number) {
  ...
  if (score >= 70) return { label: '좋은 파트너십', color: 'text-[#D4AF37]', bg: 'bg-[#D4AF37]', emoji: '🤝' }
  ...
  if (result && selectedPartner) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#0a1628] via-[#0f1d32] to-[#0a1628] pb-20">
        {/* 헤더 */}
        <div className="sticky top-0 z-10 bg-[#0a1628]/90 backdrop-blur-sm border-b border-[#D4AF37]/20 px-4 py-3 flex items-center gap-3">
          <Button variant="ghost" size="icon" className="text-[#D4AF37] hover:bg-[#D4AF37]/10" onClick={handleReset}>
            <ArrowLeft className="w-5 h-5" />
          </Button>
          <h1 className="text-lg font-bold text-[#D4AF37]">사업 궁합 분석 결과</h1>
        </div>

        <div className="max-w-lg mx-auto px-4 pt-6 space-y-5">
          {/* 점수 카드 */}
          <motion.div {...fadeUp}>
            <Card className="bg-gradient-to-br from-[#0f1d32] to-[#0a1628] border-[#D4AF37]/30 text-center overflow-hidden relative">
              ...
              <CardContent className="pt-8 pb-6 relative">
                <div className="flex items-center justify-center gap-6 mb-6">
                  <div className="text-center">
                    ...
                    <p className="text-[#D4AF37] font-medium text-sm">{result.myName}</p>
                  </div>
                  <div className="flex flex-col items-center gap-1">
                    <Briefcase className="w-5 h-5 text-[#D4AF37] animate-pulse" />
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[#D4AF37] font-medium text-sm">{result.partnerName}</p>
                  </div>
                </div>
          ...
          {result.categories?.categories && result.categories.categories.length > 0 && (
            <motion.div {...fadeUp} transition={{ delay: 0.1 }}>
              <Card className="bg-[#0f1d32]/80 border-[#D4AF37]/20">
          ...
          {result.aiAnalysis && (
            <motion.div {...fadeUp} transition={{ delay: 0.2 }}>
              <Card className="bg-[#0f1d32]/80 border-[#D4AF37]/20">
                ...
                <CardContent className="prose prose-sm prose-invert max-w-none [&_h3]:text-[#D4AF37] [&_h3]:text-sm [&_h3]:font-bold [&_h3]:mt-4 [&_h3]:mb-2 [&_p]:text-white/70 [&_p]:text-sm [&_p]:leading-relaxed [&_li]:text-white/70 [&_li]:text-sm [&_strong]:text-[#D4AF37]/90 [&_ul]:space-y-1">
          ...
          <motion.div {...fadeUp} transition={{ delay: 0.3 }}>
            <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-900/20 border border-amber-700/30">
              ...
              <p className="text-xs text-amber-400/80">
                본 분석은 사주 오행 이론 기반의 참고용입니다. 실제 사업 결정은 시장 조사, 재무 분석 등 종합적 판단이 필요합니다.
              </p>
            </div>
          </motion.div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a1628] via-[#0f1d32] to-[#0a1628] pb-20">
      ...
      <div className="sticky top-0 z-10 bg-[#0a1628]/90 backdrop-blur-sm border-b border-[#D4AF37]/20 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-[#D4AF37]">사업 궁합</h1>
          <p className="text-xs text-[#D4AF37]/60">사업 파트너와의 사주 궁합 분석</p>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 pt-6 space-y-5">
        ...
        {!isLoading && partners.length === 0 && (
          <motion.div {...fadeUp} className="text-center py-12 space-y-4">
            ...
            <p className="text-[#D4AF37]/80 font-medium">사업 파트너를 먼저 등록해주세요</p>
            <p className="text-[#D4AF37]/40 text-sm">인연 관리에서 사업 파트너의 생년월일을 등록하면<br />사업 궁합을 분석할 수 있습니다.</p>
            <Button
              variant="outline"
        ...
        {isCalculating && (
          <motion.div {...fadeUp} className="text-center py-16 space-y-4">
            ...
            <p className="text-[#D4AF37]/80 font-medium">사업 궁합을 분석하고 있습니다...</p>
            <p className="text-[#D4AF37]/40 text-xs">만세력, 오행, 십성, 대운을 종합 분석 중</p>
          </motion.div>
        )}
        ...
        {!isLoading && !isCalculating && partners.length > 0 && (
          <>
            <div className="text-center space-y-2 pb-2">
              <p className="text-[#D4AF37]/60 text-sm">{myName}님의 사업 파트너 궁합을 확인하세요</p>
            </div>
            ...
            >
              {partners.map((partner) => (
                <motion.div key={partner.id} variants={fadeUp}>
                  <button
                    className="w-full text-left transition-all rounded-xl border border-[#D4AF37]/15 bg-[#0f1d32]/50 hover:border-[#D4AF37]/40 hover:bg-[#0f1d32]/80"
                  ...
                  >
                    <div className="p-4 flex items-center gap-3">
                      <div className="w-11 h-11 rounded-full bg-[#D4AF37]/10 flex items-center justify-center flex-shrink-0">
                        <Briefcase className="w-5 h-5 text-[#D4AF37]/50" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="font-semibold text-sm text-[#D4AF37]/80">{partner.name}</span>
                          <Badge variant="outline" className="text-[10px] px-1.5 py-0 text-[#D4AF37]/50 border-[#D4AF37]/20">
                            {partner.relationship}
                          </Badge>
                        </div>
                        <p className="text-[10px] text-[#D4AF37]/40">{partner.birth_date}</p>
                      </div>
                      <TrendingUp className="w-4 h-4 flex-shrink-0 text-[#D4AF37]/30" />
                    </div>
                  </button>
                </motion.div>
              ))}
            ...
            <div className="text-center pt-4">
              <Button
                variant="outline"
                size="sm"
                className="border-[#D4AF37]/20 text-[#D4AF37]/60 hover:bg-[#D4AF37]/10 text-xs"
                onClick={() => router.push('/protected/family')}
              >
                + 새 파트너 등록하기
              </Button>
