...
export default function ServiceControlPage() {
        ...
        const loadSettings = async () => {
            ...
            try {
                const { data, error } = await supabase
                    ...
                    .select("key, value")
                    .in("key", FEATURES.map(f => f.key));

                if (error) throw error;

                const loadedConfigs: Record<string, FeatureConfig> = {};
                data?.forEach(row => {
                    loadedConfigs[row.key] = typeof row.value === 'string'
                ...
                console.error("Service Control 설정 로드 실패:", errorMessage);
    ...
    const handleToggle = async (key: string, current: boolean) => {
        ...
        try {
            ...
            const errorMessage = e instanceof Error ? e.message : String(e);
            console.error("Service Control 설정 저장 실패:", errorMessage);
                            ...
                            <div className="relative flex items-center justify-between p-4 md:p-6 gap-4">
                                <div className="space-y-1 flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        {isMaintenance && <AlertTriangle className="w-3.5 h-3.5 md:w-4 md:h-4 text-red-500 flex-shrink-0" />}
                                        <span className={cn(
                                        ...
                                        {config.isActive && !isMaintenance && (
                                            <Badge variant="outline" className="text-[9px] md:text-[10px] border-gold-500/30 text-gold-400 bg-gold-500/10">LIVE</Badge>
                                        )}
                                        {config.isActive && isMaintenance && (
                                            <Badge variant="destructive" className="text-[9px] md:text-[10px] animate-pulse bg-red-500/20 text-red-400 border-red-500/30">차단 중</Badge>
                                        )}
                                    </div>
