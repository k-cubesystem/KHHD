    ...
    >
      ...
      <div className="relative flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <p className="text-[10px] md:text-xs text-ink-primary/55 font-medium tracking-wide">{label}</p>
        ...
        <p className="text-base md:text-xl font-black text-ink-primary font-serif tracking-tight">{value}</p>
        {sub && <p className="text-[10px] text-ink-primary/40">{sub}</p>}
      </div>
    </Card>
  )
}
...
function SectionTitle({ icon: Icon, children }: { icon: React.ElementType; children: React.ReactNode }) {
  return (
    <h2 className="text-base md:text-lg font-bold mb-3 md:mb-4 text-ink-primary font-serif flex items-center gap-2">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-1">
        <h1 className="text-xl md:text-2xl font-black text-ink-primary font-serif">모니터링</h1>
        <p className="text-xs md:text-sm text-ink-primary/40">
          실시간 서비스 현황 · AI 사용량 · 매출 추이를 확인합니다.
        </p>
      </div>
        ...
        <Card className="relative p-4 md:p-6 bg-gradient-to-br from-surface/30 to-surface/20 border border-white/[0.08] overflow-hidden">
          ...
          {gemini.avgLatencyByType.length > 0 ? (
            ...
            <div className="h-[220px] flex items-center justify-center">
              <p className="text-ink-primary/40 text-sm">데이터 없음</p>
            </div>
          )}
        </Card>
            ...
            {topCategories.map((cat, idx) => {
              ...
              return (
                <div key={cat.category} className="flex items-center gap-3 group">
                  <span className="text-xs text-ink-primary/40 w-4 text-right">{idx + 1}</span>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-ink-primary/70 font-medium">
                        {labels[cat.category] || cat.category}
                      </span>
                      <span className="text-xs text-ink-primary/55 font-mono">
