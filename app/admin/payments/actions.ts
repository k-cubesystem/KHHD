...
export async function getPayments(page: number = 1, limit: number = 20, statusFilter: string = "all"): Promise<{ data: AdminPayment[], total: number }> {
    ...
    try {
        ...
        if (paymentsError) {
            console.error("getPayments: DB Error", paymentsError);
        ...
        if (userIds.length > 0) {
            ...
            if (profilesError) {
                console.warn("getPayments: Profile fetch error (possibly email column or RLS), retrying simplified.", profilesError);
            ...
            if (!profilesError && profiles) {
                profilesMap = profiles.reduce((acc: Record<string, any>, curr: any) => {
                    acc[curr.id] = curr;
                    return acc;
        ...
        console.error("getPayments Critical Error:", e);
