...
export async function createInviteCode(userId: string): Promise<{ success: boolean; code?: string; error?: string }> {
    ...
    if (membersError || !members) {
        return { success: false, error: "본인 정보가 등록되지 않았습니다. 먼저 가족 관리에서 본인 정보를 등록해주세요." };
    }
...
// 초대 코드로 초대자 정보 가져오기
export async function getInviterByCode(code: string): Promise<{ success: boolean; inviter?: InviteData; error?: string }> {
    const supabase = await createClient();
