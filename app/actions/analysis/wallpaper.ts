...
export interface WallpaperStatus {
  ...
  /** 활성 멤버십(해지 뒤 남은 기간 포함) — 있으면 전 장이 열린다. */
  isMember: boolean
  /** 내가 이미 연 장(예전 소장·광고·용신 선물). */
...
const UNLOCK_SOURCES: readonly WallpaperUnlockSource[] = ['purchase', 'ad', 'saju']

function toUnlockRecord(row: UnlockRow): WallpaperUnlockRecord | null {
  const source = UNLOCK_SOURCES.find((s) => s === row.source)
  return source ? { wallpaperId: row.wallpaper_id, source } : null
}
...
/**
 * 판정에 필요한 것을 모은다 — 현황과 광고 해금이 **같은 근거**를 보게 하는 단일 지점.
 *
...
async function loadAccessContext(userId: string) {
  ...
  // 해금 목록이 비면 «아직 아무것도 안 연 사람»으로 보일 뿐, 광고·선물 경로는 그대로 열려 있다.
  if (unlockError) logger.warn('[wallpaper] unlocks load failed:', unlockError)
  // 용신은 부가 정보 — 없으면 「내 오행」 배지·선물만 빠지고 화면은 성립한다.
  ...
  const giftedElement = giftedId ? (PREMIUM_WALLPAPER_SET.find((item) => item.id === giftedId)?.element ?? null) : null
  ...
  return {
    ...
    access: {
      ...
      // 🔴 선물은 **한 번 정해지면 고정**이다(2026-08-26). 근거인 두 표가 모두 사용자가 직접 쓸 수
      //    있는 자리(analysis_history 는 본인 행 INSERT 가능, user_energy_profile 은 본인 행 UPDATE
      //    가능)라, 매 호출 재계산하면 용신을 바꿔가며 오행 5장을 전부 가져갈 수 있다. 이미 선물로 받은
      //    장이 있으면 그 오행만 인정한다 — 위조하더라도 «원래 받았을 1장»을 넘지 못한다.
      //    현황(getWallpaperStatus)도 이 함수를 거친다 — 현황만 재계산하던 구멍이 있었다.
      myElement: giftedElement ?? yongsin,
 ...
 *    이달의 복은 크론이 공개 Storage 버킷에 올리므로 사정이 같다. 여섯 장은 전부 무료라 잃는 것이 없다.
 */
export async function getWallpaperStatus(): Promise<WallpaperStatus | null> {
  ...
  const { monthly, rows, yongsin, access } = await loadAccessContext(user.id)
  ...
  return {
    ...
    unlocks: [...access.unlocks],
    adUsedToday: rows.some((r) => r.source === 'ad' && kstDateKey(new Date(r.created_at)) === today),
    monthly,
...
async function signPremiumUrls(access: WallpaperAccess, userId: string): Promise<Record<string, string>> {
  ...
  if (!access.unlocks.some((u) => u.source === 'saju')) {
...
/**
 * 광고 보고 한 장 열기 — 하루 1장. 무료 세트와 「채운」 프리미엄 모두 같은 길이다.
 *
...
export async function unlockWallpaperByAd(wallpaperId: string): Promise<WallpaperUnlockResult> {
  ...
  // 무료 세트 → 프리미엄 순으로 찾는다(id 는 겹치지 않는다). 클라가 보낸 id 는 세트에 있을 때만 믿는다.
