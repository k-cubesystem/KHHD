...
import { sajuAnalysisSchema } from '@/lib/validations/analysis'
...
- 이 섹션이 결과의 맨 앞에 와야 합니다 — 사용자가 "이걸 어떻게 알지?" 하는 소름 경험을 만드세요
- 위 '과거 대운 상세' 데이터를 반드시 참조하여, 대운 천간이 일간을 충하거나 형하는 시점, 세운(해당 연도 간지)이 원국과 충돌하는 시점을 정밀하게 역산하세요

## 현재 공감 (두 번째)
지금 이 사람이 겪고 있을 상황을 정확히 짚어주세요.
- 현재 대운과 세운이 만들어내는 에너지 상태를 구체적으로 서술
- "지금 ~하고 계시죠" 수준의 공감 표현 사용
- 명리학적 근거 (현재 세운에서 어떤 십성이 어떻게 작용하는지)
- 구체적 행동 조언 포함 ("11월까지는 ~에 집중하세요")

## 미래 처방 (세 번째)
- 월별 하이라이트를 "7월 셋째 주에 중요한 결정을 내리세요" 수준의 구체성으로
- 행동 처방 포함 ("빨간색 소품을 놓으세요", "서쪽 방향 피하세요")
- 시적 표현을 활용한 전환 ("봄비 뒤의 무지개처럼, 4월 이후 운이 풀립니다")

## 톤 가이드 (7:3 법칙)
- 긍정적 메시지 70%, 주의/경고 30%
- 전문 용어에 반드시 괄호 설명 추가: "편재(偏財, 돌아다니는 재물)"
- 시적 표현과 현대적 비유 적극 활용
...
[출력 데이터 구조 (JSON Mandatory)]
{
    ...
    "events": [
      {
        ...
        "basis": "명리학적 근거 — 어떤 대운 천간이 일간을 충/합/형했는지, 세운과의 관계"
      },
      {
        "period": "YYYY~YYYY년",
  ...
  "currentSituation": {
    "description": "지금 이 사람이 겪고 있을 상황을 2인칭으로 공감하여 서술 (3~4문장, '지금 ~하고 계시죠' 식)",
    "basis": "현재 세운/대운에서 어떤 십성이 어떻게 작용하는지 명리학적 근거",
  ...
  "yearFortune2026": {
    ...
    "monthlyHighlights": [
      {"month": "1~2월", "keyword": "핵심 키워드", "advice": "구체적 주 단위 행동 지침 (예: '2월 둘째 주에 새로운 제안을 하세요')", "actionItem": "실행 가능한 행동 1가지 (예: '빨간색 소품 착용')"},
      {"month": "3~4월", "keyword": "핵심 키워드", "advice": "구체적 행동 지침", "actionItem": "실행 가능한 행동"},
      {"month": "5~6월", "keyword": "핵심 키워드", "advice": "구체적 행동 지침", "actionItem": "실행 가능한 행동"},
      {"month": "7~8월", "keyword": "핵심 키워드", "advice": "구체적 행동 지침", "actionItem": "실행 가능한 행동"},
      {"month": "9~10월", "keyword": "핵심 키워드", "advice": "구체적 행동 지침", "actionItem": "실행 가능한 행동"},
      {"month": "11~12월", "keyword": "핵심 키워드", "advice": "구체적 행동 지침", "actionItem": "실행 가능한 행동"}
    ]
  },
...
[작성 원칙 — 반드시 준수]
1. 과거→현재→미래 순서: pastRetrograde를 가장 먼저 분석하고, currentSituation으로 공감한 뒤, 나머지 분석을 진행하세요.
2. 현대적 해석: "운명의 기운", "하늘이 정한" 같은 무속 표현 금지. "~합니다/~입니다" 체로 명확하게 서술.
3. 장점·단점 확실하게(7:3 법칙): 긍정 70%, 주의 30%. 좋은 말만 하지 마세요. 강점은 구체적 상황과 함께 설명하고, 약점은 실제 발생하는 문제 상황과 보완법까지 반드시 포함.
   - 나쁜 예: "리더십이 강합니다" → 좋은 예: "팀을 이끌 때 빠른 의사결정력이 빛나지만, 타인의 의견을 충분히 듣지 않아 독단적이라는 평가를 받을 수 있습니다. 중요한 결정 전 최소 2명의 의견을 구하는 습관을 들이세요."
4. 사주 용어 사용 시: 반드시 괄호 안에 현대적 설명 추가. 예: "식신(食神, 표현력과 창의적 아웃풋을 뜻하는 기운)"
5. 시기 분석: "7월 셋째 주", "9월 초순" 수준의 구체적 시기를 명시하세요. "언젠가", "조만간" 같은 모호한 표현 금지.
6. 행동 처방: 모든 섹션에서 "그래서 어떻게 해야 하는가"를 반드시 포함. "빨간색 착용 추천", "서쪽 방향 피하세요" 같은 실행 가능한 처방.
7. 시적 전환: 섹션 사이에 시적 비유로 자연스럽게 연결하세요. "봄비 뒤의 무지개처럼", "겨울 끝자락의 매화처럼"
8. 분량: personality, lifeTimeline, career, love 섹션은 각각 충분히 상세하게 작성. 짧은 한 줄 답변 금지.
9. 점수(score) 필드는 절대 포함하지 마세요.`
    )

    const aiResult = await generateAIContent({
      featureKey: 'saju_detail',
      systemPrompt:
        '당신은 30년 경력의 사주명리학 전문가입니다. 백운산·강헌·도원 수준의 깊이 있는 인생 풀이를 제공합니다. 당신의 핵심 능력은 "과거를 먼저 맞추고, 현재를 공감한 뒤, 미래를 처방하는 것"입니다. 대운과 세운의 충(衝)/합(合)/형(刑) 발생 시점을 역산하여 과거 사건을 정확히 추론하고, 이를 통해 사용자의 신뢰를 얻은 뒤 미래를 제시합니다. 무속적 표현 대신 현대적 언어로, 긍정 70% + 주의 30%의 균형 잡힌 분석을 합니다. 반드시 유효한 JSON만 출력하십시오.',
      userPrompt: prompt,
      maxTokens: 8192,
    })
